...
# how to import the or_ and and_ functions from sqlalchemy
...
def part1_register():
    ...
    try:
        ...
        user_type = "user"
        new_user = User(username=username, password_hash=hash_password(password), user_type=user_type)
...
def part3_vulnerable():
    ...
    try:
        ...
        if sp_d == "custom":
            if sp_date_range:
                if sp_date_range == "-1":
                    ...
                    query = query.filter(Book.book_date >= datetime.datetime.now() - datetime.timedelta(days=int(sp_date_range)))
        elif sp_d == "specific":
            try:
                if sp_start_month and sp_start_day and sp_start_year:
                    start_date = datetime.datetime(int(sp_start_year), int(sp_start_month), int(sp_start_day))
                    if sp_end_month and sp_end_day and sp_end_year:
                        end_date = datetime.datetime(int(sp_end_year), int(sp_end_month), int(sp_end_day))
                        query = query.filter(Book.book_date >= start_date, Book.book_date <= end_date)
                    else:
        ...
        if sp_m == "0":
            query = query.with_entities(Book.title, Book.authors, Book.category, Book.price, Book.book_date)
        ...
        if sp_d == "custom":
            if sp_date_range:
                if sp_date_range == -1:
                    ...
                    query = query.filter(Book.book_date >= datetime.datetime.now() - datetime.timedelta(days=sp_date_range))
        elif sp_d == "specific":
            try:
                if sp_start_month and sp_start_day and sp_start_year:
                    start_date = datetime.datetime(sp_start_year, sp_start_month, sp_start_day)
                    if sp_end_month and sp_end_day and sp_end_year:
                        end_date = datetime.datetime(sp_end_year, sp_end_month, sp_end_day)
                        query = query.filter(Book.book_date >= start_date, Book.book_date <= end_date)
                    else:
                        return "Invalid end date"
                else:
